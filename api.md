...
| Method | Path | Description |
| ------ | ---- | ----------- |
| GET | `/api/analytics/summary?date=YYYY-MM-DD` | Single-request summary tiles for a given day (default today). Returns `{peak_hour, present_today, unknowns_today}`. `peak_hour` is the local-time hour bucket with the most distinct people spotted (e.g. `"09:00 – 10:00"`), or `null` if no data. `present_today` is the count of distinct known persons with visits. `unknowns_today` is the count of `unknown_N` folders currently in `faces/` — i.e. unresolved auto-captured persons regardless of when they were last seen. |
